...
RERANK_MODEL = "cross-encoder/ms-marco-MiniLM-L-6-v2"  # Critical for precision
FAISS_INDEX_PATH = "faiss_index"

# Initialize the embedder ONCE with the correct BGE settings
...
class DocumentReranker:
    """Critical for improving Token Grounding - ensures top-5 are actually relevant"""
...
def generate_hypothetical_document(query: str, llm) -> str:
    ...
    try:
        ...
        if "  <think>  " in hyde_text and " " in hyde_text:
            hyde_text = re.sub(r'.*?', '', hyde_text, flags=re.DOTALL)
...
class EvidenceAttributor:
    """Replaces SHAP with efficient cross-encoder attribution."""
...
class TokenAttributor:
    """Improved with stemming and substring matching for better grounding scores"""
    
    def __init__(self):
        # Optional: import stemming for better matching
        try:
            from nltk.stem import PorterStemmer
    ...
    def attribute(self, answer: str, context_docs: List[Document]) -> Dict:
        ...
        # Also create stemmed context for fuzzy matching
        ...
        for token in tokens:
            ...
            # SCORING LOGIC (Less strict than before):
            ...
            elif len(token) <= 2:  # Punctuation/small words
                score = 1.0  # Assume grounded for punctuation
            else:
        ...
        return {
            ...
            "ungrounded_tokens": [h["token"] for h in heatmap if not h["is_grounded"]][:15]
        }
...
def compute_faithfulness(answer: str, context_docs: List[Document]) -> float:
    ...
    try:
        ...
        return GenerationMetrics.bertscore(answer, context_text)
    except Exception as e:
...
class HybridRetriever(BaseRetriever, BaseModel):
    ...
    def _get_relevant_documents(self, query: str) -> List[Document]:
        ...
        # PHASE 2: Rerank (This fixes your grounding score!)
...
def run_rag_pipeline(
    ...
    # 1. Calculate Retrieval Score locally
    if context_docs:
        try:
            # We reference the global 'embedder' you defined at the top of the file
            ...
            # Calculate dot product (similarity scores) and get the mean
    ...
    evidence_attr = EvidenceAttributor()
    response.evidence_attribution = evidence_attr.attribute(query, response.answer, context_docs)
    
    token_attr = TokenAttributor()
    response.token_attribution = token_attr.attribute(response.answer, context_docs)
...
def print_colored_tokens(tokens_data: List[Dict], max_display: int = 50):
    """Print tokens with ANSI color codes"""
    GREEN = '\033[92m'
    RED = '\033[91m'
    RESET = '\033[0m'
...
if __name__ == "__main__":
    ...
    # Initialize with HyDE ALWAYS ON
    hyde_llm_init = OllamaLLM(model=FALLBACK_LLM, temperature=0.1, max_tokens=256)
    retriever = get_hybrid_retriever_from_csv(
        CSV_PATH, 
        top_k=5, 
        enable_hyde=True,
        hyde_llm=hyde_llm_init
    )
    ...
    print("   'hyde status'        - Check HyDE (Always ON)")
    ...
    while True:
        try:
            ...
            if query.lower() == 'hyde status':
                print("\n✅ HyDE Status: ALWAYS ENABLED")
                continue
            ...
            # Run pipeline with STRICT grounding
            result = run_rag_pipeline(query, retriever, ground_truth=ground_truth, llm=llm, use_strict_prompt=True)
            ...
            print(f"\n📚 CITATIONS: {cm.get('unique_citations', 0)} unique, {cm.get('valid_citation_rate', 0)*100:.0f}% valid")
            
            ta = result.token_attribution
            print(f"\n🔍 TOKEN GROUNDING ({ta.get('grounding_ratio', 0)*100:.1f}% grounded):")
